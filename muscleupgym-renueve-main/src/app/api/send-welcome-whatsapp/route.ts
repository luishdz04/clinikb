...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: user, error: userError } = await supabaseAdmin
      .from('profiles')
      .select('first_name, last_name, whatsapp, email, contract_pdf_url')
      .eq('id', userId)
      .single();
    ...
    console.log("Template ID:", process.env.TWILIO_TEMPLATE_ID);

    // Enviar mensaje usando la plantilla aprobada
    try {
      // Intentamos enviar con variables 1 y 2 (Nombre y Link/Email)
      // Si la plantilla requiere más, fallará, pero cubrimos lo más común.
      const variables = JSON.stringify({
        "1": fullName,
        "2": user.contract_pdf_url || "https://muscleupgym.com" // Fallback si no hay contrato aún
      });
      console.log("Variables de contenido:", variables);
